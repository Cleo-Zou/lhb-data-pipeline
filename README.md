...
| **附件 `lhb_YYYYMMDD.parquet`** | 当日原始数据，约 20 KB，给 pandas 用 |
...
- **数据字段**：`stock_lhb_detail_em`（东方财富）返回
  ...
  看板按 `龙虎榜净买额` 降序。相比新浪接口，东方财富多了「涨跌幅」与「上榜后N日
  涨跌幅」，且「上榜原因」对创业板/科创板（20% 板）股票也完整返回——新浪对这些票
  给 NaN。「上榜后N日」是上榜后的未来涨跌幅，最新几天因未来尚未发生而为空，
  需回补才能填上。
